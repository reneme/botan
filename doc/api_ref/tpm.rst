...
Once a ``Context`` is created, the Botan-based crypto backend may be enabled on
it the ``Context::use_botan_crypto_backend`` method. This will only succeed if
the method ``Context::supports_botan_crypto_backend`` returns true.
