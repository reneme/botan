...
*     2023,2026 René Meusel, Rohde & Schwarz Cybersecurity
...
void TLS_Handshake_Test::go() {
   ...
   if(m_expect_handshake_completion) {
      ...
      m_results.test_bool_eq(
         "client session resumption", m_client_cb->summary().was_resumption(), m_expect_session_resumption);
      m_results.test_bool_eq(
         "server session resumption", m_server_cb->summary().was_resumption(), m_expect_session_resumption);
   }
